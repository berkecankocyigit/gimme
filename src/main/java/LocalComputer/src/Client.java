        ...
        new Thread(() -> {
            while (true) { // Continuous listening loop
                try {
                    ...
                    if (receivedObject instanceof JobPrototypeLocal) {
                        JobPrototypeLocal job = (JobPrototypeLocal) receivedObject;
                        System.out.println("Received Job:");
                        System.out.println(job.toString()); // Call printAll to display job details
                    } else {
